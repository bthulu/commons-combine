...
import bthulu.commons.combine.text.WildcardMatcher;
...
public class FileTreeWalker {
	...
	private static Stream<File> getStream(File rootDir) {
		return StreamSupport
				.stream(fileTraverser().depthFirstPreOrder(rootDir).spliterator(), false);
	}
	...
	public static List<File> listFileWithExtension(final File rootDir,
		...
		return getStream(rootDir)
				.filter(k -> k.isFile() && Objects.equals(extension,
						FileUtil.getFileExtension(extension)))
	...
	public static List<File> listFileWithWildcardFileName(final File rootDir,
		...
		return getStream(rootDir)
				.filter(k -> WildcardMatcher.match(k.getName(), fileNamePattern))
				.collect(Collectors.toList());
	}
	...
	public static List<File> listFileWithRegexFileName(final File rootDir,
		...
		return getStream(rootDir).filter(
				k -> k.isFile() && rootDir.getName().matches(regexFileNamePattern))
				.collect(Collectors.toList());
	}
	...
	public static List<File> listFileWithAntPath(final File rootDir,
		...
		return getStream(rootDir)
				.filter(k -> WildcardMatcher.matchPath(k.getAbsolutePath(),
