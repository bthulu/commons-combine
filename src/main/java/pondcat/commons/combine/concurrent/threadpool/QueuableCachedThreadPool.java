package pondcat.commons.combine.concurrent.threadpool;
...
public final class QueuableCachedThreadPool
	...
	public QueuableCachedThreadPool(int corePoolSize, int maximumPoolSize,
			long keepAliveTime, TimeUnit unit, ControllableQueue workQueue,
	...
	/**
	 * Executes the given command at some time in the future. The command may execute in a
	 * new thread, in a pooled thread, or in the calling thread, at the discretion of the
	 * <tt>Executor</tt> implementation. If no threads are available, it will be added to
	 * the work queue. If the work queue is full, the system will wait for the specified
	 * time and it throw a RejectedExecutionException if the queue is still full after
	 * that.
	...
	public void execute(Runnable command, long timeout, TimeUnit unit) {
		...
		try {
			...
			// whether the pool is full, not for a
			// exception purpose
	...
	protected static class ControllableQueue extends LinkedBlockingQueue<Runnable> {
		...
		public boolean force(Runnable o, long timeout, TimeUnit unit)
			...
			return super.offer(o, timeout, unit); // forces the item onto the queue, to be
