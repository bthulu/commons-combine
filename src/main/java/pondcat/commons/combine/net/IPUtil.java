...
public class IPUtil {
	...
	private static @Nonnull byte[] ipv4StringToBytes(String ipv4Str) {
		Preconditions.checkArgument(ipv4Str == null || ipv4Str.isEmpty() || ipv4Str.length() > 15,
				"illegal ipv4: %s", ipv4Str);

		String[] split = StringUtils.split(ipv4Str, ".");
		Preconditions.checkArgument(split != null && split.length == 4, "illegal ipv4: %s", ipv4Str);

		byte[] byteAddress = new byte[4];
		for (int i = 0; i < 4; i++) {
			int tempInt = Integer.parseInt(split[i]);
			Preconditions.checkArgument(tempInt > -1 && tempInt < 256, "illegal ipv4: %s", ipv4Str);
