...
import pondcat.commons.combine.Pair;
...
/**
 * 实现深度的BeanOfClasssA<->BeanOfClassB复制, 仅复制名字, 类型相同的字段.
...
public abstract class BeanUtil {
	...
	/**
	 * 从source拷贝名称类型相同的属性至target, 并返回target.
	 * 性能是apache commons或spring的BeanUtils#copyProperties的30倍以上.
	 * @param source 源
	 * @param target 目标
	...
	public static <S, T> T copyProperties(S source, T target) {
		if (BeanCopierHolder.copierType == 1) {
			...
			net.sf.cglib.beans.BeanCopier copier = (net.sf.cglib.beans.BeanCopier) create(source.getClass(),
					target.getClass());
			copier.copy(source, target, null);
		}
		return target;
	 ...
	 * 性能较{@link #copyProperties(Object, Object)}弱, 建议仅在无public构造函数时使用.
	 * 注意: clazz必须有无参构造函数, 否则抛异常.
	 * @param source 源
	 * @param clazz 目标所属类
