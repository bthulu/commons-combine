 ...
 */
package pondcat.commons.combine.collection.primitive;
 ...
 * Collisions are resolved using linear probing. Deletions implement compaction, so cost
 * of remove can approach O(N) for full maps, which makes a small loadFactor recommended.
 *
...
public class LongObjectHashMap<V> implements LongObjectMap<V> {
	...
	public LongObjectHashMap(int initialCapacity, float loadFactor) {
		if (loadFactor <= 0.0f || loadFactor > 1.0f) {
			...
			// using a bigger loadFactor would trigger rehashing before the desired load
			// is reached.
	 ...
	 * @return the index where the key was found, or {@code -1} if no entry is found for
	 * that key.
	 */
	...
	/**
	 * Removes entry at the given index position. Also performs opportunistic, incremental
	 * rehashing if necessary to not break conflict chains.
	...
	/**
	 * Helper method called by {@link #toString()} in order to convert a single map key
	 * into a string. This is protected to allow subclasses to override the appearance of
	 * a given key.
	 */
	...
	private final class KeySet extends AbstractSet<Long> {
		...
		public boolean retainAll(Collection<?> retainedKeys) {
			...
			for (Iterator<PrimitiveEntry<V>> iter = entries().iterator(); iter
					.hasNext();) {
		...
		public void remove() {
			if (prevIndex == -1) {
				throw new IllegalStateException(
						"next must be called before each remove.");
			}
			if (removeAt(prevIndex)) {
				...
				// should instead point to the prevIndex which now holds an element which
				// was "moved back".
				nextIndex = prevIndex;
			}
			prevIndex = -1;
